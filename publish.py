...
def generate_slug(title, folder_path=None):
    ...
    if folder_path:
        ...
        path_parts = [slugify(folder) for folder in folder_path]
        return "/".join(path_parts) + "/" + title_slug
...
def generate_htaccess_redirects(moved_notes, output_dir):
    """Generate .htaccess redirect rules for moved notes."""
    htaccess_path = Path(output_dir) / ".htaccess"

    # Read existing .htaccess if present
    existing_content = ""
    if htaccess_path.exists():
        with open(htaccess_path, "r") as f:
            existing_content = f.read()

    # Generate new redirect rules
    new_rules = []
    for moved in moved_notes:
        old_url = f"{moved['old_path']}.html"
        new_url = f"{moved['new_path']}.html"
        rule = f"Redirect 301 {old_url} {new_url}"
    ...
    if new_rules:
        with open(htaccess_path, "a") as f:
            if existing_content and not existing_content.endswith("\n"):
                f.write("\n")
            f.write("# Redirects generated by publish.py\n")
...
def main():
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force publish even if notes have moved (generates redirects)",
    )
    ...
    try:
        ...
        all_notes = export.get_notes_from_folder_recursive(conn, folder_id)
    finally:
        conn.close()
    ...
    if moved_notes:
        print("Warning: The following notes have moved:", file=sys.stderr)
        for moved in moved_notes:
            print(f"  '{moved['title']}':", file=sys.stderr)
            print(
                f"    {moved['old_path']}.html -> {moved['new_path']}.html",
                file=sys.stderr,
            )

        if not args.force:
            print(
                "\nThis will break existing links to these articles.",
                file=sys.stderr,
            )
            print("Use --force to proceed and generate redirects.", file=sys.stderr)
            sys.exit(1)
        else:
            # Generate redirects for moved notes
            redirect_count = generate_htaccess_redirects(moved_notes, output_dir)
            print(f"\nGenerated {redirect_count} redirect(s) in .htaccess")
    ...
    if last_published:
        ...
        # Also include moved notes (they need to be republished at new location)
        moved_identifiers = {m["identifier"] for m in moved_notes}
        for note in all_notes:
            if note["identifier"] in moved_identifiers and note not in notes:
                notes.append(note)
    ...
    for note in all_notes:
        ...
        if slug in slug_to_notes:
            ...
            print(
                f"Error: Slug collision detected for '{slug}':",
                file=sys.stderr,
            )
            print(
                f"  - '{existing['title']}' (ID: {existing['identifier']})",
                file=sys.stderr,
            )
            print(f"  - '{note['title']}' (ID: {note['identifier']})", file=sys.stderr)
            print(
                "Notes in the same folder cannot have the same title.", file=sys.stderr
            )
            sys.exit(1)
        slug_to_notes[slug] = note

    # Process notes that need HTML regeneration (modified notes)
    # But build snippets for ALL notes for index/RSS
    posts = []
    updated_count = 0
    ...
    if last_published:
        ...
        print(f"Index and RSS include {len(posts)} total posts")
    else:
